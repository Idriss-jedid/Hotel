...
public class HotelRoomDAO {
    ...
    public List<HotelRoom> getHotelsWithDetails(String city, int minStars, String roomType, int maxCapacity, double minPrice, double maxPrice) {
        ...
        String query = "SELECT h.name AS hotel_name, h.stars AS number_of_stars, h.city AS city, rt.label AS room_type, rt.prix AS price, rt.capacity AS capacity, h.image AS image_url FROM Hotel h JOIN HotelRoom hr ON h.id = hr.hotel_id JOIN RoomType rt ON hr.room_type_id = rt.id WHERE h.city = ? AND h.stars >= ? AND rt.capacity <= ? AND rt.label = ? AND rt.prix BETWEEN ? AND ?";
